          ...
          {!showConfirmation ? (
            <div className="mt-auto flex flex-col gap-3 sm:flex-row">
              ...
              >
                {isLoading
                  ? "Behandler..."
                  : keepButtonText || "Behold Abonnementet"}
              </Button>
              <Button
                variant="destructive"
                className="flex-1"
                ...
                disabled={isLoading}
              >
                {continueButtonText || "Avslutt abonnement"}
              </Button>
            </div>
          ) : (
            <div className="mt-auto flex flex-col gap-4">
              <div className="bg-muted/50 rounded-lg p-4 text-center">
                <h3 className="text-foreground mb-2 font-semibold">
                  {finalTitle || "Bekreft avsluttelse"}
                </h3>
                <p className="text-muted-foreground mb-2 text-sm">
                  {finalSubtitle ||
                    "Er du sikker på at du vil avslutte abonnementet ditt?"}
                </p>
                <p className="text-destructive text-sm">
                  {finalWarningText ||
                    "Denne handlingen kan ikke angres og du vil miste tilgangen til alle premiumfunksjoner."}
                </p>
              </div>
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                >
                  {isLoading
                    ? "Avslutter..."
                    : confirmButtonText || "Ja, avslutt abonnementet"}
                </Button>
              </div>
            </div>
          )}
