...
const footerWrapperVariants = cva(
  "flex items-center justify-between rounded-md p-6",
  {
    variants: {
      variant: {
        small: "p-4",
...
export function PricingTableThree({
  ...
  function calculateDiscount(
    monthlyPrice: string | number | undefined,
    yearlyPrice: string | number | undefined,
  ): number {
    const monthly = parseFloat(String(monthlyPrice ?? ""));
    const yearly = parseFloat(String(yearlyPrice ?? ""));

    const monthlyLower =
      typeof monthlyPrice === "string"
        ? monthlyPrice.toLowerCase()
        : String(monthlyPrice ?? "").toLowerCase();
    const yearlyLower =
      typeof yearlyPrice === "string"
        ? yearlyPrice.toLowerCase()
        : String(yearlyPrice ?? "").toLowerCase();
        ...
        <div className="flex justify-center">
          {yearlyPriceDiscount > 0 && (
            <span className="text-muted-foreground mt-2 text-xs">
              Spar {yearlyPriceDiscount}% med årlig abonnement.
            </span>
          )}
        </div>
      </div>
                ...
                <AnimatePresence mode="wait">
                  {isAnnually ? (
                    ...
                    >
                      ...
                      <p className="text-muted-foreground">per år</p>
                    </motion.div>
                  ) : (
                    <motion.div
                      key="monthly"
